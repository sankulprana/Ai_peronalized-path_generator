...
export const toggleTaskCompletion = async (req, res, next) => {
  try {
    ...
    // Search task inside phases with string & ObjectId fallback
    ...
    // Save roadmap (triggers pre-save hook for progress percent recalculation)
    await roadmap.save();

    // Reward XP to user if task was marked completed
    const user = await User.findById(req.user._id);
    if (user) {
      if (!previousState && targetTask.completed) {
        user.xp += targetTask.xp;
        // Simple level calculation (Every 300 XP = 1 Level)
